	...
	<view>
		...
		<template v-else>
			...
			<view :key="updateKey"
				...
				role="tree" 
				name="LyTreeExpand">
				<u-tree2-node v-for="nodeId in childNodesId" 
					:nodeId="nodeId" 
					:render-after-expand="renderAfterExpand"
					:show-checkbox="showCheckbox" 
					:show-radio="showRadio" 
					:check-only-leaf="checkOnlyLeaf"
					:check-only-index="checkOnlyIndex"
...
<script>
	...
	// import LyTreeNode from './u-tree2-node.vue';

	export default {
		name: 'uTree2',
		
		componentName: 'uTree2',
		...
		props: {
			...
			// 是否展开当前节点下的所有子节点
			isExpendCurrentNodeTree: Boolean,
			...
			// 是否某一个节点才显示单选/多选框，配合checkIndex
			checkOnlyIndex: {
				type: Boolean,
				default: false
			},

			// 是否某一个节点才显示单选/多选框，配合checkOnlyIndex
			checkIndex: {
				type: Number,
				default: 1
			},
		...
		watch: {
			...
			checkStrictly(newVal) {
				this.store.checkStrictly = newVal || this.checkOnlyLeaf || this.checkOnlyIndex;
			},
			...
			*/
			...
			init() {
				...
				this.store = new TreeStore({
					...
					props: props,
					load: this.load,
					isExpendCurrentNodeTree: this.isExpendCurrentNodeTree,
					leafNodeClick: this.leafNodeClick,
					rightTextClick: this.rightTextClick,
					showCheckbox: this.showCheckbox,
					showRadio: this.showRadio,
					currentNodeKey: this.currentNodeKey,
					checkStrictly: this.checkStrictly || this.checkOnlyLeaf || this.checkOnlyIndex,
