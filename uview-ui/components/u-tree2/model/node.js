...
export default class Node {
	constructor(options) {
		...
		this.labelStyle = getPropertyFromData(this, 'labelStyle') || {};
		this.lineVisible = getPropertyFromData(this, 'lineVisible') || false;
		this.textVisible = getPropertyFromData(this, 'textVisible') || false;
		this.richText = getPropertyFromData(this, 'richText') || '';
		this.textStyle = getPropertyFromData(this, 'textStyle') || {};
		this.page = getPropertyFromData(this, 'page');
		this.canLoadMore = getPropertyFromData(this, 'canLoadMore');
	...
	expand(callback, expandparent) {
		const done = () => {
			...
			if (this.store().isExpendCurrentNodeTree) {
				if (this.childNodesId && this.childNodesId.length) {
					this.childNodesId.forEach(item => {
						const node = this.store().getNode(item);
						if (node) node.expand()
					});
				}
			}
	...
	updateLeafState() {
		...
		if (!this.store().lazy || (this.store().lazy === true && this.loaded === true)) {
			...
			// 一级节点不可能成为叶节点
			this.isLeaf = !childNodesId
			if (this.level > 1) {
				this.isLeaf = childNodesId.length === 0
			}
			return;
		}
		this.isLeaf = false;
	...
	loadMoreData(callback, defaultProps = {}) {
		if (this.store().lazy === true &&
			this.store().load && this.canLoadMore &&
			(!this.loading || Object.keys(defaultProps).length)
		) {
			this.loading = true;
			...
			this.store().load(this, resolve, true);
		} else {
			callback && callback.call(this);
		}
	}
